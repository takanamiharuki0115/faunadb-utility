...
const queryTermsByFaunaIndexes = async (clientOrToken: Client | string, indexes: string, terms: any[], consoleLog = false as boolean) => {
  let client: Client
  ...
  return client
    .query(faunaQuery.Paginate(faunaQuery.Match(faunaQuery.Index(indexes), ...terms)))
    .then((response: any) => {
...
export default queryTermsByFaunaIndexes
